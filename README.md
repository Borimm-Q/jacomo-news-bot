...
- **소스**: 거래소 공식 공지(바이낸스·OKX·KuCoin) + 코인 매체 RSS(CoinDesk·Cointelegraph·The Block·Decrypt·토큰포스트·블록미디어) + 매크로 뉴스(Finnhub)
  → **전체 목록·엔드포인트·제외 사유는 [SOURCES.md](SOURCES.md) 참고**
- **가공**: 구글 제미나이(무료 티어)가 사실만 추출해 한국어로 새로 작성 + 분류/필터 (**API 비용 $0**)
- **발행**: 텔레그램 봇이 지정 토픽에 자동 게시 (완전 자동)
- **구동**: GitHub Actions + cron-job.org (15분마다, 무료)
