"""구글 제미나이(Gemini, 무료 티어)로 수집 항목을 가공합니다.

역할:
1) 발행 여부 판단(필터) — 가격 급등락 단순 알림, 자두두 관련, 광고/사기성, 무의미 항목 제외
2) 사실만 추출해 한국어로 재작성 (원문 문장 복제 금지 = 저작권 안전)
3) 카테고리·종류(news/analysis) 분류
...
def _call_gemini(system: str, user: str, max_tokens: int) -> str:
    """제미나이 REST 호출. 응답 텍스트(JSON 문자열)를 반환. 실패/차단 시 빈 문자열."""
    body = {
...
def process(item: dict) -> dict | None:
    ...
    try:
        raw = _call_gemini(_SYSTEM, user_msg, 600)
    except Exception as exc:  # noqa: BLE001 - 어떤 API 오류든 이 항목만 건너뜀
        print(f"[rewrite] Gemini 호출 실패(건너뜀): {exc}")
        return None
    if not raw:
        return None
