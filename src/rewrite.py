...
2. 한국어로만 쓴다. 문체는 신문 속보식 '음슴체(명사형 종결)'로 간결하게 써라.
   예) '합의했다'→'합의함', '발표했다'→'발표', '중단한다'→'중단', '인상했다'→'인상', '~로 보인다'→'~로 보임'.
3. 'news'(사실 보도) 항목에는 우리 해석을 덧붙이지 마라. 일어난 사실만 음슴체로 전한다.
   ('~로 보임', '~할 전망', '시장에 호재/악재' 같은 해석성 서술을 사실 뉴스에 붙이지 말 것)
...
아무 설명 없이 아래 JSON 하나만 출력해라:
{
  ...
  "title_ko": "한 줄 한국어 제목(음슴체)",
  "summary_ko": "2~3문장 한국어 요약(음슴체)",
